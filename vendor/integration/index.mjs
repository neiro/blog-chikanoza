...
import configBuilder from './utils/configBuilder.mjs';
      ...
      'astro:build:done': async ({ logger }) => {
        ...
        try {
          ...
          const hasIntegration =
            Array.isArray(cfg?.integrations) && cfg.integrations?.some((entry) => entry?.name === '@astrojs/sitemap');
          ...
          if (hasIntegration && sitemapExists) {
            ...
            const content = pattern.test(robotsTxt)
              ? robotsTxt.replace(pattern, `Sitemap: ${sitemapUrl}`)
              : `${robotsTxt}${os.EOL}${os.EOL}Sitemap: ${sitemapUrl}`;
            fs.writeFileSync(robotsTxtFileInOut, content, { encoding: 'utf8', flag: 'w' });
          }
        } catch {
          buildLogger.debug('Unable to update robots.txt with the sitemap URL.');
        }
      },
    },
  };
